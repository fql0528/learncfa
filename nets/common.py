...
#    Basic layer
##############################
def act_layer(act_type, inplace=False, neg_slope=0.05, n_prelu=1):#原版neg_slope=0.2
...
class ResidualDenseBlock5(nn.Module):
    ...
    def __init__(self, nc, gc=32, kernel_size=3, stride=1, bias=True,
        ...
        # self.conv5 = ConvBlock(nc+4*gc, gc, kernel_size, stride, bias=bias, norm_type=norm_type,
                            #    act_type=act_type) #原版
        self.conv5 = ConvBlock(nc+4*gc, nc, kernel_size, stride, bias=bias, norm_type=norm_type,
                               act_type=act_type)

    def forward(self, x):
        x1 = self.conv1(x)
        # print('x.shape',x.shape) #torch.Size([8, 64, 256, 256])  
        # print('x1.shape', x1.shape) #torch.Size([8, 64, 256, 256])
        x2 = self.conv2(torch.cat((x, x1), 1))
        # print('torch.cat((x, x1), 1).shape',torch.cat((x, x1), 1).shape) #torch.Size([8, 128, 256, 256])
        # print('x2.shape', x2.shape) #torch.Size([8, 64, 256, 256])
        x3 = self.conv3(torch.cat((x, x1, x2), 1))
        # print('torch.cat((x, x1, x2), 1).shape',torch.cat((x, x1, x2), 1).shape) #torch.Size([8, 192, 256, 256])
        # print('x3.shape',x3.shape) #torch.Size([8, 64, 256, 256])
        x4 = self.conv4(torch.cat((x, x1, x2, x3), 1))
        # print('torch.cat((x, x1, x2, x3), 1).shape', torch.cat((x, x1, x2, x3), 1).shape) #torch.Size([8, 256, 256, 256])
        # print('x4.shape',x4.shape) #torch.Size([8, 64, 256, 256])
        x5 = self.conv5(torch.cat((x, x1, x2, x3, x4), 1))
        # print('torch.cat((x, x1, x2, x3, x4), 1).shape', torch.cat((x, x1, x2, x3, x4), 1).shape) #torch.Size([8, 320, 256, 256])
        # print('x5.shape',x5.shape) #torch.Size([8, 64, 256, 256])
        # exit()
