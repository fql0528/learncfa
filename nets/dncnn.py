...
class DnCNN(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=64, nb=17, act_mode='BR', shutter_lengths=None, shutter_name=None):
        """
        # ------------------------------------
        ...
        self.shutter_lengths = shutter_lengths
        self.shutter_name = shutter_name

    def forward(self, x, shutter_lengths=None):
        n = self.model(x)

        if x.shape[1] > 1:
            # quad exposure agg order is L,M,S
            # only do subtraction on short exposure
            if 'learn_all' in self.shutter_name:
                img = x[:, 0:1, :, :] - n
            else:
                img = x[:, 3:4, :, :] - n
