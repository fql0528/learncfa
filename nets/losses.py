...
class LPIPS_1D(nn.Module):
    def __init__(self):
        super().__init__()
        # self.perceptual_loss = lpips.LPIPS(net='vgg').cuda() #原版
...
class L2LPIPSRegLoss(nn.Module):
    ...
    def forward(self, y_pred, gt):
        # print('y_pred.shape',y_pred.shape) #torch.Size([1, 4, 512, 512])
...
class TwoL2Loss(nn.Module):
    def __init__(self):
        super().__init__()
        self.l2_loss = nn.MSELoss()
    
    def forward(self,restored,gt,w,rgb_to_one):
        
        return 100*self.l2_loss(restored,gt)+self.l2_loss(w,rgb_to_one)